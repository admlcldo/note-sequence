...
# reads the individual notes
def read_notes(track):
    notes = []
    for msg in track:
        if msg.type == 'note_on' and msg.velocity > 0:
            notes.append(str(msg.note % 12))
    return notes
...
# converts midi file to type 0 by using the merge_tracks function
...
# convenience function for reading notes or chords
def read_midi(file_path, mode='chords'):
    if mode == 'chords':
        return read_midi_file(file_path)
    elif mode == 'notes':
        try:
            mid = mido.MidiFile(file_path)
            print('Reading', file_path)
